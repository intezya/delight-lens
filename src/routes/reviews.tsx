...
export const Route = createFileRoute("/reviews")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Все распарсенные отзывы с тональностью, темами и AI-разбором.",
      },
    ],
  }),
      ...
      <div className="motion-page mx-auto w-full max-w-[1440px] space-y-4 px-3 py-4 sm:px-4 md:px-6">
        ...
        <div className="stagger grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <button
            ...
            className={`motion-surface press rounded-xl border p-3 text-left transition ${sentiment === "positive" ? "border-positive/50 bg-positive-soft/40" : "border-border bg-card hover:bg-positive-soft/30"}`}
          >
            <p className="text-[10px] uppercase tracking-wider text-positive-foreground">Позитив</p>
            <p className="num mt-1 display text-2xl font-semibold text-positive-foreground">
              {counts.pos}
            </p>
          </button>
          <button
            onClick={() => setSentiment("mixed")}
            className={`motion-surface press rounded-xl border p-3 text-left transition ${sentiment === "mixed" ? "border-mixed/50 bg-mixed-soft/40" : "border-border bg-card hover:bg-mixed-soft/30"}`}
          >
            <p className="text-[10px] uppercase tracking-wider text-mixed-foreground">Смешанный</p>
            <p className="num mt-1 display text-2xl font-semibold text-mixed-foreground">
              {counts.mix}
            </p>
          </button>
          <button
            onClick={() => setSentiment("negative")}
            className={`motion-surface press rounded-xl border p-3 text-left transition ${sentiment === "negative" ? "border-negative/50 bg-negative-soft/40" : "border-border bg-card hover:bg-negative-soft/30"}`}
          >
            <p className="text-[10px] uppercase tracking-wider text-negative-foreground">Негатив</p>
            <p className="num mt-1 display text-2xl font-semibold text-negative-foreground">
              {counts.neg}
            </p>
          </button>
        </div>
