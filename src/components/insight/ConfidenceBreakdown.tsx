    ...
    <Card className="motion-surface p-6 md:p-7">
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          <p className="text-[10px] uppercase tracking-wider text-muted-foreground">Уверенность системы</p>
          <div className="flex items-baseline gap-2">
            <span className="display num text-4xl font-semibold tabular-nums">{value}%</span>
            <span className="text-xs text-muted-foreground">на основе {breakdown.reviewsCount} отзывов</span>
          </div>
          <p className="mt-2 max-w-2xl text-sm leading-relaxed text-foreground/80">
            Оценка собрана из {breakdown.factors.length} факторов: объёма данных, повторяемости, тональности,
            разнообразия источников и свежести сигнала.
          </p>
        </div>
      </div>
          ...
          <li key={f.label} className="motion-row flex items-start gap-2.5 rounded-lg border bg-muted/30 p-3">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center justify-between gap-2">
                ...
                <span className="num text-[11px] font-medium tabular-nums text-muted-foreground">+{f.score}</span>
              </div>
