...
export function ImpactTracker({ tracking, predicted }: { tracking: ImplementationTracking; predicted?: string }) {
  ...
  const data = Array.from({ length: 12 }).map((_, i) => {
    const t = i / 11;
    const v = tracking.before.complaintsPerWeek - (tracking.before.complaintsPerWeek - tracking.after.complaintsPerWeek) * t;
    ...
    <Card className="motion-surface overflow-hidden p-0">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b bg-positive-soft/30 px-6 py-4">
        <div className="flex items-center gap-2">
          ...
          <p className="text-sm font-semibold">Гипотеза внедрена · {format(new Date(tracking.implementedAt), "d MMMM yyyy", { locale: ru })}</p>
        </div>
      ...
      <div className="grid gap-0 md:grid-cols-[1.2fr_1fr]">
        <div className="grid grid-cols-2 gap-0 border-b md:border-b-0 md:border-r">
          ...
          <Stat label="Жалоб в неделю · после" value={tracking.after.complaintsPerWeek} positive last />
          <Stat label="Доля негатива · до" value={`${tracking.before.negativeShare}%`} last />
          <Stat label="Доля негатива · после" value={`${tracking.after.negativeShare}%`} positive last />
        </div>
        <div className="p-5">
          ...
          <div className="motion-chart h-[120px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={data}>
                ...
                <Area type="monotone" dataKey="v" stroke="var(--positive)" strokeWidth={2} fill="url(#impact-grad)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
          <div className="mt-2 flex items-center justify-between text-[11px]">
            <span className="text-muted-foreground">Прогноз: <span className="font-medium text-foreground">{predicted ?? "—"}</span></span>
            <span className="text-muted-foreground">Факт: <span className="font-semibold text-positive">−{drop}%</span></span>
          </div>
        </div>
      </div>
    </Card>
...
function Stat({ label, value, positive, last }: { label: string; value: number | string; positive?: boolean; last?: boolean }) {
  return (
    <div className={`p-5 ${last ? "" : "border-b"}`}>
      <p className="text-[10px] uppercase tracking-wider text-muted-foreground">{label}</p>
      <p className={`display num mt-1 text-2xl font-semibold tabular-nums ${positive ? "text-positive" : "text-foreground"}`}>{value}</p>
    </div>
  );
}
