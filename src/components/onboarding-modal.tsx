...
const STEPS = [
  {
    icon: Tags,
    title: "Откройте тему",
    text: "Зайдите в раздел «Темы» — там сгруппированы проблемы и возможности, которые AI находит в отзывах.",
  },
  {
    icon: Lightbulb,
    title: "Прочитайте гипотезу",
    text: "Каждая тема содержит гипотезы вида «если..., то..., потому что...» — это конкретное предположение, что нужно изменить.",
  },
  {
    icon: Quote,
    title: "Проверьте доказательства",
    text: "У каждой гипотезы — список реальных отзывов с подсветкой ключевых фрагментов и ссылкой на источник.",
  },
  {
    icon: ShieldCheck,
    title: "Оцените уверенность и риски",
    text: "Уверенность системы показывает, насколько надёжен сигнал. Рядом — что именно может оказаться не так.",
  },
  {
    icon: Send,
    title: "Передайте команде",
    text: "Если гипотеза подтвердилась — нажмите «Передать в работу». Если данных мало — «Нужны данные».",
  },
] as const;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <ol className="guide-dialog-steps mt-2 space-y-3">
          {STEPS.map((s, i) => (
            <li
              key={s.title}
            ...
            >
              ...
              <div className="flex-1">
                <p className="text-sm font-medium">
                  <span className="text-muted-foreground">Шаг {i + 1} · </span>
                  {s.title}
                </p>
